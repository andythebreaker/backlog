<template>
  <div
    :class="{ doneItem: isDone, isEditing: isEditing }"
  ...
  >
    ...
    <div class="item-div" v-else>
      <Checkbox :value="isDone" @on-change="changeIsDone"> </Checkbox>
      <span
        class="item-text"
        v-html="textWithLink"
...
export default {
  ...
  methods: {
    ...
    addEmoji({ emoji }) {
      ...
      const afterText = this.draftText.slice(
        this.caretPosition,
        this.draftText.length
      );
